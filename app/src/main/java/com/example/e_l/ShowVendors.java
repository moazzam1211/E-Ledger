...
public class ShowVendors extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RecyclerView vendorsList = (RecyclerView) findViewById(R.id.vendorsList);
        vendorsList.setLayoutManager(new LinearLayoutManager(this));

        reference = FirebaseDatabase.getInstance().getReference();
        reference.keepSynced(true);
        reference.child("User").child("Vendors").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot ds:snapshot.getChildren()){
                    try {
                        ...
                        phonear.add(number);
                        vendorsList.setAdapter(new VendorsListAdapter(namear,comar,addar,emailar,phonear));
